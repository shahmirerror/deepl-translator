...
// Function to parse Markdown-like content into a nested array
...
// Call the parsing function to parse the Markdown-like content
$result = Yaml::parse($markdown);
        ...
        foreach ($value as $childKey => $childValue) {
          if(is_array($childValue))
          {
            ...
            foreach ($childValue as $gchildKey => $gchildValue) {
              ...
              // $properties[$gchildKey] = translate($gchildKey, $gchildValue, $exclusions, $exclusions2, $apiKey, $target_language);  
            }
          }
          else
          {
            ...
            // $properties[$childKey] = translate($childKey, $childValue, $exclusions, $exclusions2, $apiKey, $target_language);
          }
        }
    } else {
...
// Function to convert array to Markdown-like format
function arrayToMarkdown($array, $depth = 0) {
    $markdown = '';
    $spaces = str_repeat('    ', $depth);

    foreach ($array as $key => $value) {
        if (is_array($value)) {
            $markdown .= $spaces . $key . ':' . PHP_EOL . arrayToMarkdown($value, $depth + 1);
        } else {
            $markdown .= $spaces . $key . ': ' . $value . PHP_EOL;
        }
    }
...
// Print the resulting data array in Markdown-like format
// echo arrayToMarkdown($result);
?>
